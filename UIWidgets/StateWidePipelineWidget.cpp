...
// Constructor
StateWidePipelineWidget::StateWidePipelineWidget(QWidget *parent, VisualizationWidget* visWidget, QString assetType, QString appType) : AssetInputWidget(parent, visWidget, assetType, appType)
{
#ifdef OpenSRA
...
void StateWidePipelineWidget::handleLoadStateData(void)
{
    ...
    this->statusMessage(pathToOSRABackend);

    // Assemble the path for the
    auto path = pathToOSRABackend + QDir::separator() + "lib"+QDir::separator()+"OtherData"+QDir::separator()+"Preprocessed"+QDir::separator()+"CA_Natural_Gas_Pipeline_Segments_WGS84";

    auto currPath = componentFileLineEdit->text();
